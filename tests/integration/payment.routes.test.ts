...
describe("payment route boundaries", () => {
  it("keeps payment details and administration behind authentication", async () => {
    const detail = await request(app).get("/api/v1/payments/a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11").expect(401);
  ...
  it("rejects unsigned Stripe webhook calls", async () => {
    const result = await request(app).post("/api/v1/payments/webhooks/stripe").send({ id: "evt_test" }).expect(400);
