...
describe("Phase 6 service authorization", () => {
  ...
  it("blocks doctors from booking before persistence", async () => {
    await expect(
      service.book(input, { userId: "doctor-user", role: "DOCTOR", profileId: input.doctorId }, {}),
  ...
  it("blocks cross-patient and non-admin emergency booking", async () => {
    await expect(
      service.book(
        input,
        { userId: "patient-user", role: "PATIENT", profileId: "other-patient" },
        {},
      ),
    ).rejects.toMatchObject({ statusCode: 403, code: "FORBIDDEN" });
    await expect(
      service.book(
        ...
        { userId: "patient-user", role: "PATIENT", profileId: input.patientId },
        {},
      ),
    ).rejects.toMatchObject({ statusCode: 403, code: "FORBIDDEN" });
  });
