...
import { authorize, authorizeExact } from "../../src/app/middleware/rbac.middleware.js";
...
const authenticatedAs =
  (role: ApplicationRole): RequestHandler =>
  (req, _res, next) => {
    req.auth = {
...
describe("service-layer ownership policy", () => {
  ...
  it("allows access by user or profile ownership", () => {
    expect(assertResourceOwnership(patient, { ownerUserId: "patient-user" })).toEqual({
      usedAdminOverride: false,
    });
    expect(assertResourceOwnership(patient, { ownerProfileId: "patient-profile" })).toEqual({
      usedAdminOverride: false,
    });
  });

  it("blocks cross-user access", () => {
    expect(() => assertResourceOwnership(patient, { ownerUserId: "another-user" })).toThrowError(
  ...
  it("allows auditable admin override and supports disabling it", () => {
    ...
    expect(assertResourceOwnership(admin, { ownerUserId: "patient-user" })).toEqual({
      usedAdminOverride: true,
    });
    expect(() =>
      assertResourceOwnership(admin, {
