...
describe("Phase 6 appointment contracts", () => {
  it("validates booking IDs, notes, and emergency flag", () => {
    ...
    expect(bookAppointmentSchema.safeParse({ ...ids, notes: "x".repeat(1001) }).success).toBe(
      false,
    );
  });
  ...
  it("validates doctor filters and date ranges", () => {
    expect(doctorAppointmentQuerySchema.parse({ date: "2026-09-01" })).toMatchObject({
      page: 1,
      limit: 20,
    });
    expect(
      doctorAppointmentQuerySchema.safeParse({ startDate: "2026-09-02", endDate: "2026-09-01" })
        .success,
    ).toBe(false);
    expect(doctorAppointmentQuerySchema.safeParse({ paymentStatus: "UNKNOWN" }).success).toBe(
      false,
    );
  });
  ...
  it("validates combined admin filters and fee ranges", () => {
    expect(
      adminAppointmentQuerySchema.safeParse({
        minFee: "100",
        maxFee: "500",
        specialty: "Cardiology",
      }).success,
    ).toBe(true);
    expect(adminAppointmentQuerySchema.safeParse({ minFee: "500", maxFee: "100" }).success).toBe(
      false,
    );
