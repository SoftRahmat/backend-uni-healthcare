...
} from "../../src/app/module/appointment/appointment-payment.service.js";
import {
  adminPaymentActionSchema,
  paymentHistoryQuerySchema,
  refundPaymentSchema,
} from "../../src/app/module/payment/payment.validation.js";
...
describe("Phase 7 payment policies", () => {
  ...
  it("uses the non-network fallback when Stripe is not configured", async () => {
    const payment = await new StripeCheckoutPaymentGateway().pending({
      paymentId: "payment",
      appointmentId: "appointment",
      amount: 500,
      patientId: "patient",
  ...
  it("validates bounded history filters and positive refunds", () => {
    expect(
      paymentHistoryQuerySchema.parse({ page: "2", limit: "50", status: "PAID" }),
    ).toMatchObject({ page: 2, limit: 50, status: "PAID" });
  ...
  it("requires justification for auditable administrator overrides", () => {
    ...
    expect(
      adminPaymentActionSchema.parse({
        action: "ADD_NOTE",
        notes: "Reviewed",
        justification: "Reviewed against bank settlement",
      }).action,
    ).toBe("ADD_NOTE");
  });
});
