...
import type { AppointmentStatus, PaymentStatus, Prisma, UserRole } from "../../../generated/prisma/client.js";
...
export const isValidAppointmentTransition = (from: AppointmentStatus, to: AppointmentStatus): boolean =>
...
export const calculateCancellation = (
  ...
  if (actorRole === "PATIENT" && hoursBeforeStart < 24) {
    throw new ApiError(400, "Patients must cancel at least 24 hours before the appointment", "CANCELLATION_WINDOW_CLOSED");
  }
  if (actorRole === "DOCTOR" && hoursBeforeStart < 12) {
    throw new ApiError(400, "Doctors must cancel at least 12 hours before the appointment", "CANCELLATION_WINDOW_CLOSED");
  }
  ...
  const refundType = refundAmount === amount && amount > 0 ? "FULL"
    : refundAmount > 0 ? "PARTIAL" : "NONE";
  const nextPaymentStatus: PaymentStatus = paymentStatus === "PENDING" ? "FAILED"
    : refundType === "FULL" ? "REFUNDED"
      : refundType === "PARTIAL" ? "PARTIAL_REFUND" : paymentStatus;
...
const scheduleDateWhere = (startDate?: string, endDate?: string): Prisma.ScheduleWhereInput | undefined =>
  startDate || endDate ? { scheduleDate: { gte: startDate ? dbDate(startDate) : undefined, lte: endDate ? dbDate(endDate) : undefined } } : undefined;
...
export class AppointmentService {
  ...
  async book(input: BookAppointmentInput, actor: AppointmentActor, context: RequestContext, now = new Date()) {
    if (actor.role !== "PATIENT" && !isAdmin(actor.role)) {
      throw new ApiError(403, "Only patients and administrators can book appointments", "FORBIDDEN");
    }
    ...
    const result = await prisma.$transaction(async (transaction) => {
      ...
      const locked = await transaction.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "schedules" WHERE "id" = ${input.scheduleId} FOR UPDATE`;
      ...
      const [patient, doctor, schedule] = await Promise.all([
        ...
        transaction.schedule.findFirst({
          where: {
            id: input.scheduleId, isDeleted: false, isBooked: false,
      ...
      if (!patient.healthData) throw new ApiError(400, "Complete patient health data before booking", "HEALTH_DATA_REQUIRED");
      if (!doctor) throw new ApiError(400, "Doctor is inactive or unavailable", "DOCTOR_UNAVAILABLE");
      if (!schedule) throw new ApiError(409, "Schedule is unavailable or already booked", "SCHEDULE_ALREADY_BOOKED");
      const { startsAt } = appointmentWindow(schedule);
      if (startsAt <= now) throw new ApiError(400, "Past appointment slots cannot be booked", "PAST_APPOINTMENT_SLOT");
      ...
      if (!input.emergency) {
        ...
        const monthly = await transaction.appointment.count({
          where: {
            patientId: input.patientId, isDeleted: false, status: { not: "CANCELLED" },
            schedule: { scheduleDate: { gte: dbDate(monthStart), lt: nextMonth } },
          },
        });
        if (monthly >= 5) throw new ApiError(409, "Patients can book at most five appointments per month", "MONTHLY_APPOINTMENT_LIMIT");
        const doctorSpam = await transaction.appointment.findFirst({
          where: {
            patientId: input.patientId, doctorId: input.doctorId, isDeleted: false,
            status: { not: "CANCELLED" },
            schedule: {
              scheduleDate: { gte: dbDate(addIsoDays(targetDate, -7)), lte: dbDate(addIsoDays(targetDate, 7)) },
            },
          },
          select: { id: true },
        });
        if (doctorSpam) throw new ApiError(409, "The same doctor cannot be booked again within seven days", "DOCTOR_REBOOKING_LIMIT");
      }
      const sameDay = await transaction.appointment.findMany({
        where: {
          patientId: input.patientId, isDeleted: false, status: { in: activeStatuses },
          schedule: { scheduleDate: schedule.scheduleDate },
        },
        include: { schedule: true },
      });
      if (sameDay.some(({ schedule: existing }) =>
      ...
      const appointment = await transaction.appointment.create({
        data: {
          id: appointmentId, patientId: patient.id, doctorId: doctor.id, scheduleId: schedule.id,
          appointmentFee: doctor.appointmentFee, notes: input.notes, videoCallingId,
          payment: { create: {
  ...
  private async assertPatientView(patientId: string, actor: AppointmentActor) {
    if (actor.role === "PATIENT" && actor.profileId !== patientId) throw new ApiError(403, "Patients can only view their own appointments", "FORBIDDEN");
    if (actor.role === "DOCTOR") throw new ApiError(403, "Use the doctor appointment view", "FORBIDDEN");
  }

  async listPatient(patientId: string, query: PatientAppointmentQuery, actor: AppointmentActor, context: RequestContext) {
    ...
    const [appointments, total] = await prisma.$transaction([
      prisma.appointment.findMany({ where, include: appointmentInclude, skip: (query.page - 1) * query.limit, take: query.limit, orderBy: { createdAt: "desc" } }),
      prisma.appointment.count({ where }),
    ]);
    await prisma.auditLog.create({ data: { action: "APPOINTMENT_LIST_VIEWED", userId: actor.userId, ...context, metadata: { actorUserId: actor.userId, patientId } } });
    return { appointments: appointments.map((item) => listView(item, "PATIENT")), meta: { page: query.page, limit: query.limit, total, totalPages: Math.ceil(total / query.limit) } };
  }

  async listDoctor(doctorId: string, query: DoctorAppointmentQuery, actor: AppointmentActor, context: RequestContext) {
    ...
    const dateWhere = query.date ? { scheduleDate: dbDate(query.date) } : scheduleDateWhere(query.startDate, query.endDate);
    const where: Prisma.AppointmentWhereInput = {
      doctorId, isDeleted: false,
      ...(query.status?.length ? { status: { in: query.status } } : {}),
      ...(dateWhere ? { schedule: dateWhere } : {}),
      ...(query.patientSearch ? { patient: { name: { contains: query.patientSearch, mode: "insensitive" } } } : {}),
      ...(query.paymentStatus ? { payment: { status: query.paymentStatus } } : {}),
    };
    const [appointments, total] = await prisma.$transaction([
      prisma.appointment.findMany({
        where, include: appointmentInclude, skip: (query.page - 1) * query.limit, take: query.limit,
        orderBy: [{ schedule: { scheduleDate: "asc" } }, { schedule: { startTime: "asc" } }],
      }),
      prisma.appointment.count({ where }),
    ]);
    await prisma.auditLog.create({ data: { action: "APPOINTMENT_LIST_VIEWED", userId: actor.userId, ...context, metadata: { actorUserId: actor.userId, doctorId } } });
    ...
    return { appointments: views, groupedByDate, meta: { page: query.page, limit: query.limit, total, totalPages: Math.ceil(total / query.limit) } };
  }

  async getById(appointmentId: string, actor: AppointmentActor, context: RequestContext, now = new Date()) {
    const appointment = await prisma.appointment.findFirst({ where: { id: appointmentId, isDeleted: false }, include: appointmentInclude });
    if (!appointment) throw new ApiError(404, "Appointment was not found", "APPOINTMENT_NOT_FOUND");
    const owns = (actor.role === "PATIENT" && actor.profileId === appointment.patientId)
      || (actor.role === "DOCTOR" && actor.profileId === appointment.doctorId) || isAdmin(actor.role);
    ...
    await prisma.auditLog.create({ data: { action: "APPOINTMENT_DETAIL_VIEWED", userId: actor.userId, ...context, metadata: { actorUserId: actor.userId, appointmentId } } });
    return {
      ...listView(appointment, actor.role === "DOCTOR" ? "DOCTOR" : isAdmin(actor.role) ? "ADMIN" : "PATIENT"),
      videoCallLink,
      prescription: null,
      review: null,
      cancellation: appointment.cancelledAt ? {
        cancelledAt: appointment.cancelledAt, cancelledByUserId: appointment.cancelledByUserId,
        cancelledByRole: appointment.cancelledByRole, reason: appointment.cancellationReason,
        refundType: appointment.refundType, refundAmount: appointment.refundAmount,
      } : null,
    };
  }

  async updateStatus(appointmentId: string, input: UpdateAppointmentStatusInput, actor: AppointmentActor, context: RequestContext, now = new Date()) {
    if (input.status === "CANCELLED") return this.cancel(appointmentId, {}, actor, context, now);
    if (actor.role === "PATIENT") throw new ApiError(403, "Patients may only cancel scheduled appointments", "FORBIDDEN");
    const appointment = await prisma.appointment.findFirst({ where: { id: appointmentId, isDeleted: false }, include: appointmentInclude });
    if (!appointment) throw new ApiError(404, "Appointment was not found", "APPOINTMENT_NOT_FOUND");
    if (actor.role === "DOCTOR" && actor.profileId !== appointment.doctorId) throw new ApiError(403, "Doctors can only update assigned appointments", "FORBIDDEN");
    if (!isValidAppointmentTransition(appointment.status, input.status)) {
      if (["COMPLETED", "CANCELLED"].includes(appointment.status)) throw new ApiError(409, "Final appointment status cannot be changed", "APPOINTMENT_FINALIZED");
      throw new ApiError(400, "Appointment status transition is invalid", "INVALID_STATUS_TRANSITION");
    }
    if (actor.role === "DOCTOR" && !["INPROGRESS", "COMPLETED"].includes(input.status)) throw new ApiError(403, "Doctor status transition is not allowed", "FORBIDDEN");
    const { startsAt } = appointmentWindow(appointment.schedule);
    if (input.status === "INPROGRESS" && now.getTime() < startsAt.getTime() - 15 * 60 * 1_000) {
      throw new ApiError(400, "Appointment cannot start more than 15 minutes early", "APPOINTMENT_TOO_EARLY");
    }
    const updated = await prisma.$transaction(async (transaction) => {
      ...
      if (!isValidAppointmentTransition(current.status, input.status)) throw new ApiError(409, "Appointment status changed concurrently", "APPOINTMENT_STATUS_CONFLICT");
      const saved = await transaction.appointment.update({
        where: { id: appointmentId },
        data: { status: input.status, ...(input.status === "INPROGRESS" ? { startedAt: now } : { completedAt: now }) },
      });
      await transaction.auditLog.create({ data: { action: "APPOINTMENT_STATUS_UPDATED", userId: actor.userId, ...context, metadata: { actorUserId: actor.userId, appointmentId, from: current.status, to: input.status } } });
      return saved;
    });
  ...
  async cancel(appointmentId: string, input: CancelAppointmentInput, actor: AppointmentActor, context: RequestContext, now = new Date()) {
    const appointment = await prisma.appointment.findFirst({ where: { id: appointmentId, isDeleted: false }, include: appointmentInclude });
    if (!appointment) throw new ApiError(404, "Appointment was not found", "APPOINTMENT_NOT_FOUND");
    const owns = (actor.role === "PATIENT" && actor.profileId === appointment.patientId)
      || (actor.role === "DOCTOR" && actor.profileId === appointment.doctorId) || isAdmin(actor.role);
    if (!owns) throw new ApiError(403, "You cannot cancel this appointment", "FORBIDDEN");
    if (appointment.status !== "SCHEDULED") throw new ApiError(409, "Only scheduled appointments can be cancelled", "APPOINTMENT_NOT_CANCELLABLE");
    const { startsAt } = appointmentWindow(appointment.schedule);
    ...
    const cancelled = await prisma.$transaction(async (transaction) => {
      await transaction.$queryRaw`SELECT "id" FROM "appointments" WHERE "id" = ${appointmentId} FOR UPDATE`;
      const current = await transaction.appointment.findUniqueOrThrow({ where: { id: appointmentId } });
      if (current.status !== "SCHEDULED") throw new ApiError(409, "Appointment status changed concurrently", "APPOINTMENT_STATUS_CONFLICT");
      const saved = await transaction.appointment.update({ where: { id: appointmentId }, data: {
        status: "CANCELLED", cancelledAt: now, cancelledByUserId: actor.userId,
        cancelledByRole: actor.role as UserRole, cancellationReason: input.reason,
        refundType: policy.refundType, refundAmount: policy.refundAmount,
      } });
      await transaction.schedule.update({ where: { id: appointment.scheduleId }, data: { isBooked: false } });
    ...
    invalidate(appointment);
    await this.emails.sendCancelled({ patientEmail: appointment.patient.email, doctorEmail: appointment.doctor.email, reason: input.reason, refundAmount: policy.refundAmount });
    return { id: cancelled.id, status: cancelled.status, refund: { type: policy.refundType, amount: policy.refundAmount, processedAt: policy.refundAmount > 0 ? now : null }, cancelledBy: actor.userId, cancelledAt: now };
  }
  ...
  async search(query: AdminAppointmentQuery, actor: AppointmentActor, context: RequestContext) {
    ...
    const doctorWhere: Prisma.DoctorWhereInput | undefined = query.doctorSearch || query.specialty ? {
      ...(query.doctorSearch ? { name: { contains: query.doctorSearch, mode: "insensitive" } } : {}),
      ...(query.specialty ? { specialties: { some: { specialty: { title: { contains: query.specialty, mode: "insensitive" } } } } } : {}),
    } : undefined;
    const where: Prisma.AppointmentWhereInput = {
      isDeleted: false,
      ...(query.status?.length ? { status: { in: query.status } } : {}),
      ...(query.patientSearch ? { patient: { OR: [
      ...
      ...(query.paymentStatus ? { payment: { status: query.paymentStatus } } : {}),
      ...(query.minFee !== undefined || query.maxFee !== undefined ? { appointmentFee: { gte: query.minFee, lte: query.maxFee } } : {}),
      ...(scheduleDateWhere(query.startDate, query.endDate) ? { schedule: scheduleDateWhere(query.startDate, query.endDate) } : {}),
    };
    const [appointments, total, groups, paid, specialtyRows] = await prisma.$transaction([
      prisma.appointment.findMany({ where, include: appointmentInclude, skip: (query.page - 1) * query.limit, take: query.limit, orderBy: { [query.sortBy]: query.sortOrder } }),
      prisma.appointment.count({ where }),
      ...
      prisma.appointment.findMany({
        where,
        select: { doctor: { select: { specialties: { select: { specialty: { select: { id: true, title: true } } } } } } },
      }),
    ]);
    ...
    await prisma.auditLog.create({ data: { action: "APPOINTMENT_LIST_VIEWED", userId: actor.userId, ...context, metadata: { actorUserId: actor.userId, adminSearch: true, resultCount: appointments.length } } });
    return {
      appointments: appointments.map((item) => listView(item, "ADMIN")),
      analytics: {
        totalAppointments: total,
        statusBreakdown: Object.fromEntries(groups.map((group) => [group.status, group._count._all])),
        totalRevenue: (paid._sum.amount ?? 0) - (paid._sum.refundAmount ?? 0),
        averageFee: paid._avg.amount ?? 0,
        popularSpecialties: [...specialtyCounts.values()].sort((left, right) => right.count - left.count).slice(0, 10),
      },
