...
export const bookAppointment = asyncHandler(async (request, response) => {
  const result = await appointmentService.book(
    bookAppointmentSchema.parse(request.body),
    actorFrom(request),
    contextFrom(request),
  );
...
export const listPatientAppointments = asyncHandler(async (request, response) => {
  const result = await appointmentService.listPatient(
    String(request.params.patientId),
    patientAppointmentQuerySchema.parse(request.query),
    actorFrom(request),
    contextFrom(request),
  );
  response
    .status(200)
    .json(successResponse("Appointments retrieved successfully", result.appointments, result.meta));
});
export const listDoctorAppointments = asyncHandler(async (request, response) => {
  const result = await appointmentService.listDoctor(
    String(request.params.doctorId),
    doctorAppointmentQuerySchema.parse(request.query),
    actorFrom(request),
    contextFrom(request),
  );
  response
    ...
    .json(
      successResponse(
        ...
        { appointments: result.appointments, groupedByDate: result.groupedByDate },
        result.meta,
      ),
    );
});
export const searchAppointments = asyncHandler(async (request, response) => {
  const result = await appointmentService.search(
    adminAppointmentQuerySchema.parse(request.query),
    actorFrom(request),
    contextFrom(request),
  );
  response
    ...
    .json(
      successResponse(
        ...
        { appointments: result.appointments, analytics: result.analytics },
        result.meta,
      ),
    );
});
...
export const updateAppointmentStatus = asyncHandler(async (request, response) => {
  const result = await appointmentService.updateStatus(
    String(request.params.appointmentId),
    updateAppointmentStatusSchema.parse(request.body),
    actorFrom(request),
    contextFrom(request),
  );
...
export const cancelAppointment = asyncHandler(async (request, response) => {
  const result = await appointmentService.cancel(
    String(request.params.appointmentId),
    cancelAppointmentSchema.parse(request.body ?? {}),
    actorFrom(request),
    contextFrom(request),
  );
