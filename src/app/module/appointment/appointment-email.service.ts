  ...
  async sendBooked(input: {
    ...
    await Promise.all([
      this.transport.send({
        ...
        ...emailShell("Appointment booked", message),
      }),
      this.transport.send({
        to: input.doctorEmail,
        subject: "New appointment",
        ...emailShell("New appointment", message),
      }),
    ]);
  }

  async sendCancelled(input: {
    patientEmail: string;
    doctorEmail: string;
    reason?: string;
    refundAmount: number;
    ...
    await Promise.all([
      this.transport.send({
        ...
        ...emailShell("Appointment cancelled", message),
      }),
      this.transport.send({
        to: input.doctorEmail,
        subject: "Appointment cancelled",
