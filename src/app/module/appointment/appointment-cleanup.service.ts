...
export const processAppointmentLifecycle = async (
  ...
  const expired = await prisma.appointment.findMany({
    where: { isDeleted: false, status: "SCHEDULED", payment: { status: "PENDING", expiresAt: { lte: now } } },
    select: {
      id: true, scheduleId: true, patientId: true, doctorId: true,
  ...
  for (const appointment of expired) {
    const changed = await prisma.$transaction(async (transaction) => {
      ...
      const current = await transaction.appointment.findUnique({ where: { id: appointment.id }, include: { payment: true } });
      if (!current || current.status !== "SCHEDULED" || current.payment?.status !== "PENDING" || current.payment.expiresAt > now) return false;
      await transaction.appointment.update({ where: { id: current.id }, data: {
        status: "CANCELLED", cancelledAt: now, cancellationReason: "Payment expired",
      } });
      await transaction.payment.update({ where: { id: current.payment.id }, data: { status: "FAILED" } });
      await transaction.schedule.update({ where: { id: current.scheduleId }, data: { isBooked: false } });
      await transaction.auditLog.create({ data: {
        action: "APPOINTMENT_PAYMENT_EXPIRED", userId: appointment.patient.userId,
    ...
    if (changed) {
      ...
      await emails.sendCancelled({
        patientEmail: appointment.patient.email,
        doctorEmail: appointment.doctor.email,
        reason: "Payment expired",
        refundAmount: 0,
      }).catch((error: unknown) => logger.error("Payment expiry notification failed", { appointmentId: appointment.id, error }));
    }
  }
  ...
  for (const appointment of candidates) {
    ...
    const threshold = appointment.status === "INPROGRESS"
  ...
  if (paymentExpired || autoCompleted) logger.info("Appointment lifecycle cleanup completed", { paymentExpired, autoCompleted });
