...
export const stripeWebhook = asyncHandler(async (request, response) => {
  ...
  const result = await paymentWebhookService.process(
    paymentWebhookService.construct(request.rawBody, signature),
  );
...
export const patientPaymentHistory = asyncHandler(async (request, response) => {
  const result = await paymentService.history(
    { patientId: String(request.params.patientId) },
    paymentHistoryQuerySchema.parse(request.query),
    actorFrom(request),
  );
  response
    .status(200)
    .json(
      successResponse(
        ...
        { payments: result.payments, totals: result.totals },
        result.meta,
      ),
    );
});
export const doctorPaymentHistory = asyncHandler(async (request, response) => {
  const result = await paymentService.history(
    { doctorId: String(request.params.doctorId) },
    paymentHistoryQuerySchema.parse(request.query),
    actorFrom(request),
  );
  response
...
export const refundPayment = asyncHandler(async (request, response) => {
  const result = await paymentService.refund(
    String(request.params.paymentId),
    refundPaymentSchema.parse(request.body),
    actorFrom(request),
    contextFrom(request),
  );
...
export const performAdminPaymentAction = asyncHandler(async (request, response) => {
  const result = await paymentService.adminAction(
    String(request.params.paymentId),
    adminPaymentActionSchema.parse(request.body),
    actorFrom(request),
    contextFrom(request),
  );
...
export const paymentDashboard = asyncHandler(async (request, response) => {
  const result = await paymentService.dashboard(
    adminPaymentDashboardQuerySchema.parse(request.query),
    actorFrom(request),
  );
...
export const downloadInvoice = asyncHandler(async (request, response) => {
  const result = await paymentService.invoice(
    String(request.params.paymentId),
    actorFrom(request),
    contextFrom(request),
  );
  response
    .status(200)
    .json(successResponse("Invoice download link generated successfully", result));
});
  ...
  if (query.format === "pdf") {
    ...
    for (const payment of result.payments)
      doc
        .fontSize(9)
        .text(
          `${payment.id} | ${payment.status} | ${payment.currency} ${payment.totalAmount} | ${payment.paidAt?.toISOString() ?? "-"}`,
        );
    doc.end();
    return;
  }
  const rows = [
    "paymentId,status,currency,amount,tax,total,refunded,paidAt",
