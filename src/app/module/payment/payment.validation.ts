...
export const paymentHistoryQuerySchema = z.object({
  ...
  minAmount: z.coerce.number().int().min(0).optional(), maxAmount: z.coerce.number().int().min(0).optional(),
});
export const paymentExportQuerySchema = paymentHistoryQuerySchema.extend({ format: z.enum(["csv", "pdf"]).default("csv") });
...
export const adminPaymentActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("MARK_PAID"), justification: z.string().trim().min(10).max(500), transactionId: z.string().trim().min(3).max(255) }),
  z.object({ action: z.literal("ADD_NOTE"), justification: z.string().trim().min(10).max(500), notes: z.string().trim().min(1).max(1000) }),
]);
export const adminPaymentDashboardQuerySchema = z.object({ startDate: date.optional(), endDate: date.optional() });
