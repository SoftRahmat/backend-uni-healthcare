...
const renderPdf = async (payment: InvoicePayment, invoiceNumber: string): Promise<Buffer> => {
  ...
  doc.moveDown().text(`Patient: ${payment.appointment.patient.name}`).text(`Doctor: ${payment.appointment.doctor.name}`);
  doc.text(`Service date: ${payment.appointment.schedule.scheduleDate.toISOString().slice(0, 10)} ${payment.appointment.schedule.startTime}`);
  doc.moveDown().text("Medical consultation", 50, doc.y, { continued: true }).text(money(payment.amount), { align: "right" });
  doc.text(`Tax (${(payment.taxRateBps / 100).toFixed(2)}%)`, 50, doc.y, { continued: true }).text(money(payment.taxAmount), { align: "right" });
  doc.moveDown().fontSize(12).text("Total", 50, doc.y, { continued: true }).text(money(payment.amount + payment.taxAmount), { align: "right" });
  doc.moveDown(2).fontSize(9).text(`Payment status: ${payment.status}`).text(`Transaction: ${payment.transactionId ?? payment.stripePaymentIntentId ?? "Pending"}`);
  doc.moveDown(4).fillColor("#666666").text("This invoice is digitally generated and retained in the PH HealthCare archive.", { align: "center" });
  doc.end();
  return complete;
};
const loadPayment = (paymentId: string) => prisma.payment.findFirst({ where: { id: paymentId, isDeleted: false }, include: paymentInvoiceInclude });
...
export class PaymentInvoiceService {
  ...
  async generate(paymentId: string, newVersion = false) {
    const existing = await prisma.invoice.findFirst({ where: { paymentId }, orderBy: { version: "desc" } });
    if (existing && !newVersion) return existing;
    const payment = await loadPayment(paymentId);
    if (!payment) throw new ApiError(404, "Payment was not found", "PAYMENT_NOT_FOUND");
    if (!["PAID", "REFUNDED", "PARTIAL_REFUND"].includes(payment.status)) throw new ApiError(409, "Only settled payments can be invoiced", "PAYMENT_NOT_SETTLED");
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        const invoice = await tx.invoice.create({ data: {
          paymentId, invoiceNumber, version, ...uploaded,
          checksum: createHash("sha256").update(body).digest("hex"),
          subtotal: payment.amount, taxRateBps: payment.taxRateBps, taxAmount: payment.taxAmount,
          totalAmount: payment.amount + payment.taxAmount,
          retentionUntil: new Date(Date.now() + 7 * 365.25 * 24 * 60 * 60 * 1_000),
        } });
        await tx.auditLog.create({ data: { action: "INVOICE_GENERATED", metadata: { paymentId, invoiceId: invoice.id, invoiceNumber, version } } });
        return invoice;
      });
    } catch (error) {
  ...
  async download(paymentId: string) {
    ...
    return { ...invoice, downloadUrl: await this.storage.signedDownload(invoice.objectKey, 24 * 60 * 60) };
  }
}
