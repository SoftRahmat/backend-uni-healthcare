...
export const uploadMedicalReport = asyncHandler(async (request, response) => {
  const report = await medicalReportService.upload(
    ...
    uploadMedicalReportSchema.parse(request.body),
    request.file,
    actorFrom(request),
    contextFrom(request),
  );
...
export const listMedicalReports = asyncHandler(async (request, response) => {
  const result = await medicalReportService.list(
    String(request.params.patientId),
    medicalReportListQuerySchema.parse(request.query),
    actorFrom(request),
    contextFrom(request),
  );
  response
    .status(200)
    .json(successResponse("Medical reports retrieved successfully", result.reports, result.meta));
});
